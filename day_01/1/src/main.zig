...
pub fn main() !u8 {
    ...
    // store difference between left/right in a new array
    var difference = std.ArrayList(u32).init(allocator);
    defer difference.deinit();

    for (left.items, right.items) |leftItem, rightItem| {
        try difference.append(absU32(leftItem, rightItem));
    }

    var combined: u32 = 0;
    for (difference.items) |item| {
        combined += item;
    }

    std.log.debug("Combined: {d}", .{combined});
